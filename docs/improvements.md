...
### Private documentation storage — 2026-09-25

- Source: user request to preserve private research, followed by clarification to save ordinary documents without manual versioning.
- Behavior: `/docs/private/` remains excluded from the main Git repository. The research is stored at the stable local path `docs/private/research/context-tools.md` and edited in place. Removed the numbered filename/version field, per-topic latest-version index, and separate private change log. `docs/README.md` explains that tracked documents use Git history while ignored private documents have no history in the main repository. Existing public build/architecture/product documentation remains tracked.
- Status: delivered as a documentation-only follow-up; supersedes the initial dated-revision convention. Research recommendations remain unimplemented. No product copy or app behavior changed.
- Validation: `zsh scripts/build-app.sh` passed using SwiftPM and macOS SDK 26.5, including bundle signature verification. The research body was preserved exactly except for the removed version field. Both remaining private documents were verified ignored and untracked; all local links resolved and numbered revisions were absent. `git diff --check` passed. The app test suite was not rerun for this documentation/ignore-only change. No running app session was restarted.
