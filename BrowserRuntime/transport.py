"""Bounded, non-replaying JSON-RPC over a single owned stdio child.

Derived from the repository's exercised research transport. Production callers
pin the upstream artifact and retain project policy at the Codex boundary.
"""
import json
import math
import os
    ...
    def rpc(self, method, params, request_id, deadline_ns, *, wire_gate=None):
        ...
        try:
            ...
            if not event.wait(remaining):
                self._fail('JSON-RPC deadline; outcome unknown; no replay')
    ...
    def initialize_mcp(self, *, expected_server_version, protocol_version='2025-11-25',
        ...
        try:
            initialized = self.rpc('initialize', {
                ...
                'clientInfo': {'name': 'context-desk-browser', 'version': '1.0.0'}},
